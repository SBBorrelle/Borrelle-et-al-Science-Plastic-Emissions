...
## proportions but not probabilities. You don't even want them to
## be probabilities really.
...
# stored in local folder because teh size of the files (> 4GB)
...
library(here)
##Note - thic code takes a couple of hours to run.
...
## setwd(here('data_sym', 'Spatial_probablity_data'))
#setwd(here('data'))
setwd("~/Dropbox/PLASTIC/Spatial-leakmap/")
##### Read in data ########
#world borders
...
setwd("~/Dropbox/R/PPEG_PlasticFutures/")
#dir.create('leak2', showWarnings=FALSE) #create folder to store the check figures
...
ggplot2::ggsave(here::here('leak', 'emissions_results.png'), g,
       width=15, height=7)
